	...
	[
		<#if data?exists>
			<#list data as item>
			{
				...
				"KD_KPP": "<#if item.properties["docpajak:KD_KPP"]?exists>${item.properties["docpajak:KD_KPP"]}</#if>",
				"KD_CABANG": "<#if item.properties["docpajak:KD_CABANG"]?exists>${item.properties["docpajak:KD_CABANG"]}</#if>",
				...
				"NO_TT": "<#if item.properties["docpajak:NO_TT"]?exists>${item.properties["docpajak:NO_TT"]}</#if>"
			}<#if item_has_next>,</#if>
			</#list>
		</#if>
	]
